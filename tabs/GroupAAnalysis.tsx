...
const SymptomStat: React.FC<{ stat: SymptomImprovementStat }> = ({ stat }) => (
     <div className="flex justify-between items-baseline py-2 border-b border-teal-200/50 last:border-b-0">
        <span className="text-sm font-medium text-slate-600">{stat.symptomName}</span>
        <span className="text-base font-semibold text-teal-700">{stat.improvementPercentage}%</span>
    </div>
);


export const GroupAAnalysis: React.FC<GroupAAnalysisProps> = ({ data }) => {
    return (
